...
export default async function LeaderboardPage() {
  const currentUser = await getOrCreateDemoUser();
            ...
            return (
              <div
                ...
                className={`flex items-center gap-3 rounded-xl border px-3 py-2.5 ${
                  isMe ? "border-primary/50 bg-primary/10" : "border-border bg-surface-2"
                }`}
              >
                <span className="w-6 text-center text-sm font-semibold text-muted">
                  {i === 0 ? <Crown className="mx-auto h-4 w-4 text-xp" /> : i + 1}
                </span>
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-surface text-xs font-bold text-foreground">
